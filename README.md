...
# Video-Captioning-with-Bi-modal-Transformer

- [ Video Captioning with Bi-modal Transformer](#video-captioning-with-bi-modal-transformer)
  - [Summary](#summary)
...
The model is eavlueated on 1600 Vimeo videos . The dataset can be found [here](https://drive.google.com/drive/folders/1ZQxRNL_BqBWNMFrQo_dDhmGgc1B0sHeE?usp=sharing)  (see `./data/available_mp4.txt` for ids). To this end, we evaluate the performance of our model against ~91 % of the validation videos. We provide the validation sets without such videos in `./data/val_*_no_missings.json`. Please see `Experiments` and `Supplementary Material` sections for details and performance of other models on the same validation sets.
